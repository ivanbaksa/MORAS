...
assert (A: forall (n : nat), NotL (repeat O n) = repeat I n).
{
  induction n0.
    - simpl. reflexivity.
...
rewrite A. unfold AddL. Search (rev ( repeat _ _ ) = _). rewrite rev_repeat.

assert (pomocna : forall (n:nat), AddLr (repeat I n) (repeat I n) I = repeat I n).
{
  intros. induction n0.
    - simpl. reflexivity.
...
assert (B : AddLr (repeat I n) (repeat I n) O = O :: repeat I (Nat.pred n)).
{
  induction n.
    - contradiction.
    - simpl. f_equal. now rewrite pomocna.
}

rewrite B.

assert (C: rev (O :: repeat I (Nat.pred n)) = repeat I (Nat.pred n) ++ [O]).
{
  simpl. f_equal. Search ( rev (repeat _ _) = repeat _ _). rewrite rev_repeat. reflexivity.
}

rewrite C.

assert (D : forall n : nat, NotL (repeat I n ++ [O]) = repeat O n ++ [I]).
{
  intros. induction n0.
    - reflexivity.
    - simpl. rewrite IHn0. reflexivity.
